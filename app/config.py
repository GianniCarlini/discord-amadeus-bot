...
class Settings:
    ...
    sapporo_codes: List[str] = field(default_factory=lambda: _split_csv("SAPPORO_CODES", "CTS"))  # <-- NUEVO
    ...
    def __post_init__(self):
        missing = []
        if not self.amadeus_client_id: missing.append("AMADEUS_CLIENT_ID")
        if not self.amadeus_client_secret: missing.append("AMADEUS_CLIENT_SECRET")
        if missing:
    ...
    @property
    def market(self) -> str: return self.amadeus_market
    @property
    def primary_currency(self) -> str: return self.amadeus_currency
    @property
    def tz(self): return pytz.timezone(self.timezone_str)
